...
@pytest.mark.asyncio
async def test_explicit_override_priority(monkeypatch, tmp_path):
    ...
    settings = await load_settings(MySettings, override_values=overrides)
...
@pytest.mark.asyncio
async def test_config_path_loading(tmp_path):
    ...
    settings = await load_settings(MySettings, config_path=str(config_dir))
...
@pytest.mark.asyncio
async def test_field_alias_and_env_prefix(monkeypatch):
    class MySettings(UnoSettings):
        fooBar: str = "abc"
        model_config = SettingsConfigDict(alias_generator=lambda f: f.upper())

    monkeypatch.setenv("FOOBAR", "from_env")
    settings = await load_settings(MySettings)
...
@pytest.mark.asyncio
async def test_env_var_pollution(monkeypatch):
    class MySettings(UnoSettings):
        foo: str = "default"
