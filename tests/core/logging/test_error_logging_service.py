...
from uno.infrastructure.logging.error_logging_service import ErrorLoggingService
...
def test_log_framework_error(caplog, error_logging_service):
    ...
    with caplog.at_level("ERROR"):
        error_logging_service.log_error(
            err, context={"user_id": 42}, trace_context={"correlation_id": "abc"}
        )
    found = None
    ...
    assert found is not None, (
        f"No log record with error_code='X123' found in: {[getattr(r, 'context', None) for r in caplog.records]}"
    )
...
def test_log_standard_exception(caplog, error_logging_service):
    ...
    for rec in caplog.records:
        ...
        if ctx and ctx.get("error_type") == "ValueError":
            ...
            break
    assert found is not None, (
        f"No log record with error_type='ValueError' found in: {[getattr(r, 'context', None) for r in caplog.records]}"
    )
