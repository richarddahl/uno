...
def apply_upcasters(event_type: type, data: dict[str, Any], from_version: int, to_version: int) -> dict[str, Any]:
    v = from_version
    while v < to_version:
...
class InventoryCreatedV2(DomainEvent):
    version = 2
    def __init__(self, item_id: str, name: str, sku: str):
        super().__init__(item_id=item_id, name=name, sku=sku)
...
def upcast_inventory_created_v1_to_v2(data: dict[str, Any]) -> dict[str, Any]:
    ...
    data = dict(data)
    data['sku'] = f"SKU-{data['item_id']}"
    return data

@pytest.mark.parametrize("from_version,event_type,from_data,to_version,to_event_cls,expected", [
    (1, InventoryCreatedV2, {"item_id": "abc", "name": "Widget"}, 2, InventoryCreatedV2, {"item_id": "abc", "name": "Widget", "sku": "SKU-abc"}),
])
def test_event_upcasting(from_version, event_type, from_data, to_version, to_event_cls, expected):
...
def test_upcaster_registry_missing():
    ...
    with pytest.raises(ValueError):
        apply_upcasters(InventoryCreatedV2, {"item_id": "abc", "name": "Widget"}, 1, 3)


def test_idempotent_upcast():
    # Upcasting from v2 to v2 should be a no-op
    data = {"item_id": "abc", "name": "Widget", "sku": "SKU-abc"}
