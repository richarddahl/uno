...
async def test_circuit_breaker_blocks_after_failures(
    ...
    middleware = CircuitBreakerMiddleware(
        logger=fake_logger,
        event_types=["FakeEvent"],
        options=CircuitBreakerState(failure_threshold=2, recovery_timeout_seconds=1000),
    )
    event = FakeEvent()
    ...
    found = any(
        "circuit open" in call[1].lower() for call in fake_logger.structured_log_calls
    )
    assert found
...
async def test_circuit_breaker_recovers_after_timeout(
    ...
    middleware = CircuitBreakerMiddleware(
        logger=fake_logger,
        event_types=["FakeEvent"],
        options=CircuitBreakerState(failure_threshold=1, recovery_timeout_seconds=0.01),
    )
    event = FakeEvent()
    context = EventHandlerContext(event=event)
