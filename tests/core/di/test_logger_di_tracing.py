...
async def test_di_logger_tracing_and_error_context():
    ...
    assert hasattr(config_result, "value")
    ...
    with logger_service.trace_scope(logger_service, trace_context=trace_context):
        ...
        log_record = logger_service._log_buffer[-1] if hasattr(logger_service, "_log_buffer") and logger_service._log_buffer else None
        if log_record:
            # Check correlation_id in log record
            assert "correlation_id" in log_record["extra"]
            assert log_record["extra"]["correlation_id"] == trace_context["correlation_id"]
