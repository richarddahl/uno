...
from uno.injection.decorators import service, singleton
from uno.injection.service_scope import ServiceScope
from uno.injection.service_collection import ServiceCollection
from uno.injection.service_provider import ServiceProvider
from uno.injection.errors import ServiceConfigurationError
...
class MessageService:
    ...
    async def send_message(self, message: str) -> None:
        ...
        await self.email_service.send_email(
            to="user@example.com", subject="New Message", body=message
        )


@service(
    interface=IEmailService, scope=ServiceScope.SINGLETON, options_type=EmailOptions
)
class EmailService:
