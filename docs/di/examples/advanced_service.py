...
from uno.infrastructure.di.decorators import (
    service, singleton, ServiceLifecycle, ServiceEventType,
    ServiceEvent, ServiceEventHandler, ServiceEventEmitter,
    ServiceVersion, ServiceComposite, ServiceConstraint,
    ServiceConstraintValidator, ServiceHealth
)
...
class DatabaseConstraintValidator(ServiceConstraintValidator):
    ...
    def validate(self, service: Any) -> None:
        ...
        if not hasattr(service, 'query'):
            raise ServiceConfigurationError("Database service must implement query method")
        if not hasattr(service, 'connection_pool'):
            raise ServiceConfigurationError("Database service must have connection pool")
...
class DatabaseService(ServiceLifecycle, ServiceEventEmitter):
    ...
    async def initialize(self) -> None:
        ...
        self.connection_pool = [f"Connection {i}" for i in range(self.options.pool_size)]
