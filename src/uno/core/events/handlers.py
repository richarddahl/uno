...
class EventHandlerRegistry:
    ...
    def register_handler(self, event_type: str, handler: EventHandler | Callable) -> None:
        """
    ...
    def register_middleware_for_event_type(self, event_type: str, middleware: EventHandlerMiddleware) -> None:
        """
    ...
    def register_middleware_from_factory(self, factory: Any, event_types: list[str] | None = None) -> None:
        """
            ...
            if event_types:
                ...
                for event_type in event_types:
                    for middleware in middleware_stack:
                        if event_type in self._middleware and self._middleware[event_type]:
    ...
    def get_middleware_for_event_type(self, event_type: str) -> list[EventHandlerMiddleware]:
        """
...
class MiddlewareChainBuilder:
    ...
    def build_chain(
        self, 
        middleware: list[EventHandlerMiddleware]
    ...
    def _create_middleware_executor(
        ...
        """
        async def execute_middleware(context: EventHandlerContext) -> Result[Any, Exception]:
            try:
    ...
    def _create_handler_executor(
        ...
        """
        async def execute_handler(context: EventHandlerContext) -> Result[Any, Exception]:
            ...
            try:
                ...
                if middleware_chain:
                    ...
                    class HandlerExecutor:
                        ...
                        async def execute(self, ctx: EventHandlerContext) -> Result[Any, Exception]:
                    ...
                    class MiddlewareProcessor:
                        ...
                        async def process(self, ctx: EventHandlerContext) -> Result[Any, Exception]:
                            return await self.middleware.process(ctx, self.next_processor)
                    ...
                    for middleware in reversed(middleware_chain):
                        middleware_processor = MiddlewareProcessor(middleware, processor)
...
def discover_handlers(
    ...
    initial_handler_count = sum(len(handlers) for handlers in registry._handlers.values())
                ...
                for item_name in dir(module):
                    try:
                        ...
                        if inspect.isclass(item):
                            ...
                            if (issubclass(item, EventHandler) and item != EventHandler 
                                    and not getattr(item, "_registered_with_discovery", False)):
                                ...
                                try:
                                    ...
                                    if event_type is None and hasattr(handler_instance, "event_type"):
                        ...
                        elif hasattr(item, "_is_event_handler") and getattr(item, "_is_event_handler", False):
