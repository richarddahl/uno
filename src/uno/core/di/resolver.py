...
from uno.core.di.parameter_resolution import (
...
from uno.core.di.type_utils import (
...
class ServiceResolver:
    ...
    def _instantiate_with_injection(
        ...
        for name, param in sig.parameters.items():
            self._logger.debug(f"DI: param '{name}', kind={param.kind}, annotation={param.annotation}, default={param.default}")
        ctor_params = {}
        for name, param in sig.parameters.items():
            ...
            if param.kind in (inspect.Parameter.VAR_POSITIONAL, inspect.Parameter.VAR_KEYWORD):
                self._logger.debug(f"DI: skipping param '{name}' of kind '{param.kind}' for {impl} (VAR_POSITIONAL/VAR_KEYWORD)")
                continue
            if name in ("args", "kwargs"):
                self._logger.debug(f"DI: skipping param '{name}' by name for {impl}")
                continue
            if param.kind not in (inspect.Parameter.POSITIONAL_OR_KEYWORD, inspect.Parameter.KEYWORD_ONLY):
                self._logger.debug(f"DI: skipping param '{name}' of kind '{param.kind}' for {impl}")
                continue
            if name in params:
                ...
                if dep_type is inspect.Parameter.empty:
                    self._logger.debug(f"DI: MISSING TYPE ANNOTATION for param '{name}' in {impl}")
                    return Failure(ServiceRegistrationError(f"Cannot resolve parameter '{name}' for {impl}: missing type annotation"))
                # Failsafe: assert that we are not processing *args/**kwargs or their names
                assert name not in ("args", "kwargs"), f"DI Failsafe: Attempted to process forbidden param name '{name}' for {impl}"
                assert param.kind not in (inspect.Parameter.VAR_POSITIONAL, inspect.Parameter.VAR_KEYWORD), f"DI Failsafe: Attempted to process forbidden param kind '{param.kind}' for '{name}' in {impl}"
                dep_result = self.resolve(dep_type, scope=scope)
                if isinstance(dep_result, Failure):
                    self._logger.debug(f"DI: FAILED to resolve dependency '{name}' for {impl}: {dep_result.error}")
                    return Failure(ServiceRegistrationError(f"Failed to resolve dependency '{name}' for {impl}: {dep_result.error}"))
                ctor_params[name] = dep_result.value
        # Failsafe: check after loop
        for name, param in sig.parameters.items():
            assert name not in ("args", "kwargs"), f"DI Failsafe: Post-loop found forbidden param name '{name}' for {impl}"
            assert param.kind not in (inspect.Parameter.VAR_POSITIONAL, inspect.Parameter.VAR_KEYWORD), f"DI Failsafe: Post-loop found forbidden param kind '{param.kind}' for '{name}' in {impl}"
        try:
            instance = impl(**ctor_params)
            self._logger.debug(f"DI: EXIT _instantiate_with_injection for {impl} SUCCESS instance={instance}")
            return Success(instance)
        except Exception as e:
            self._logger.debug(f"DI: EXIT _instantiate_with_injection for {impl} FAILURE error={e}")
            return Failure(ServiceRegistrationError(f"Failed to instantiate {impl}: {e}"))
    ...
    def _get_registration_or_error(
        ...
        if key not in self._registrations:
            ...
            if self._auto_register:
                ...
                from uno.core.di.type_utils import is_concrete_type
