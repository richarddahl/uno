...
class MigrationManager:
    ...
    async def apply_migrations(self, target_version: Optional[int] = None) -> Result[None, str]:
            ...
            async with self._connection_manager.get_connection() as session:
                ...
                for migration in pending:
                    try:
                        ...
                        return Failure(f"Failed to apply migration {migration.version}: {str(e)}")
        ...
        try:
            async with self._connection_manager.get_connection() as session:
                ...
                try:
                    ...
                    await session.execute(
                        text(f"DELETE FROM {self._config.DB_MIGRATIONS_TABLE} WHERE version = :version"),
                        {"version": version}
                    )
    ...
    async def get_migration_status(self) -> Result[list[Migration], str]:
        ...
        try:
            async with self._connection_manager.get_connection() as session:
                result = await session.execute(
                    text(f"SELECT version, applied_at FROM {self._config.DB_MIGRATIONS_TABLE}")
                )
