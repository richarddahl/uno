...
class SQLDocumentationGenerator:
    ...
    async def generate_markdown(self, output_dir: Path) -> Result[None, str]:
        ...
        try:
            ...
            if not all(isinstance(r, Success) for r in [schema_docs, table_docs, function_docs, view_docs, trigger_docs]):
    ...
    def _parse_trigger_def(self, definition: str) -> tuple[str, str, str]:
        ...
        timing = parts[parts.index("BEFORE") if "BEFORE" in parts else parts.index("AFTER")]
