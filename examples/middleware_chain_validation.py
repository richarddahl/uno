...
class SuccessHandler(EventHandler):
    ...
    async def handle(self, context: EventHandlerContext) -> Result[str, Exception]:
        ...
        self.call_count += 1
        print(
            f"✅ [{time.time():.3f}] SuccessHandler called (count: {self.call_count})"
        )
...
class FlakyHandler(EventHandler):
    ...
    async def handle(self, context: EventHandlerContext) -> Result[str, Exception]:
        ...
        if self.call_count <= self.succeed_after:
            print(
                f"❌ [{time.time():.3f}] FlakyHandler failed (attempt: {self.call_count}/{self.succeed_after + 1})"
            )
            return Failure(ValueError(f"Simulated failure {self.call_count}"))

        print(
            f"✅ [{time.time():.3f}] FlakyHandler succeeded after {self.call_count} attempts"
        )
...
class FailureHandler(EventHandler):
    ...
    async def handle(self, context: EventHandlerContext) -> Result[str, Exception]:
        ...
        self.call_count += 1
        print(
            f"❌ [{time.time():.3f}] FailureHandler called (always fails) (count: {self.call_count})"
        )
...
async def test_single_middleware():
    ...
    if (
        result.is_success
        and result.value == "success-1"
        and len(logging_middleware.logs) == 2
    ):
...
async def test_retry_middleware():
    ...
    if (
        result.is_success
        and result.value == expected_result
        and handler.call_count == expected_call_count
    ):
        print(
            f"✅ TEST PASSED: Retry middleware test succeeded in {total_duration:.3f}s"
        )
    else:
...
async def test_circuit_breaker():
    ...
    print(
        f"Circuit state: {circuit_breaker.circuit_state.get(event.event_type, 'unknown')}"
    )
    ...
    if (
        not result1.is_success
        and not result2.is_success
        and not result3.is_success
        and not result4.is_success
        and circuit_breaker.circuit_state.get(event.event_type)
    ...
    print(
        f"Results: fail={not result1.is_success}, fail={not result2.is_success}, "
...
async def test_metrics_middleware():
    ...
    )  # Long interval to avoid auto-reporting
    ...
    if metrics:
        ...
        if (
            ...
            and abs(metrics.success_rate - 50.0) < 0.1
        ):  # Should be ~50%
...
async def test_combined_middleware():
    ...
    retry_middleware = RetryMiddleware(
        options=RetryOptions(max_retries=3, base_delay_ms=50, backoff_factor=1.5)
    )
    circuit_breaker = CircuitBreakerMiddleware(
    ...
    if (
        ...
        print(
            f"✅ TEST PASSED: Combined middleware test succeeded in {total_duration:.3f}s"
        )
    else:
