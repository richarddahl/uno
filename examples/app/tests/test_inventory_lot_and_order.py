...
def fake_lot(fake_item: InventoryItem) -> InventoryLot:
    result = InventoryLot.create(lot_id="lot-1", item_id=fake_item.id, quantity=50, vendor_id="vendor-1", purchase_price=10.0)
    assert hasattr(result, 'unwrap'), f"Expected Result, got {type(result)}"
    return result.unwrap()

@pytest.fixture
def fake_order(fake_item: InventoryItem, fake_lot: InventoryLot) -> Order:
    result = Order.create(order_id="order-1", item_id=fake_item.id, lot_id=fake_lot.id, vendor_id="vendor-1", quantity=25, price=12.0, order_type="sale")
    assert hasattr(result, 'unwrap'), f"Expected Result, got {type(result)}"
    return result.unwrap()
