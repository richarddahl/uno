...
class MiddlewareChainBuilder:
    ...
    def build_chain(self, middleware_list: list[EventHandlerMiddleware]) -> Callable[[EventHandlerContext], Result[Any, Exception]]:
        ...
        for middleware in reversed(middleware_list):
            ...
            async def process(ctx: EventHandlerContext, middleware=m, next_processor=next_p) -> Result[Any, Exception]:
...
class RetryMiddleware(EventHandlerMiddleware):
    ...
    async def process(
        ...
        while True:
            ...
            print(f"Retry attempt {attempt}/{self.max_retries} after {delay_seconds}s delay")
