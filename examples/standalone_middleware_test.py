...
class TestEventHandler:
    ...
    def __init__(self, name: str = "test_handler", should_fail: bool = False, failure_count: int = 0):
        self.name = name
    ...
    async def handle(self, event: TestEvent) -> Result:
        ...
        if self.should_fail:
            return Failure(ValueError(f"Handler {self.name} failed (permanent failure)"))
        
        if self.current_failures < self.failure_count:
            self.current_failures += 1
            return Failure(ValueError(f"Handler {self.name} failed (temporary failure {self.current_failures}/{self.failure_count})"))
...
class EventHandlerMiddlewareFactory:
    ...
    def create_retry_middleware(self, options: RetryOptions | None = None) -> RetryMiddleware:
    ...
    def create_default_middleware_stack(
        ...
        retry = self.create_retry_middleware(
...
async def test_retry_middleware():
    ...
    retry_middleware = RetryMiddleware(options=retry_options, logger_factory=logger_factory)
    
    # Process the event
    result = await retry_middleware.process(
        context,
        lambda ctx: handler.handle(ctx.event)
    )
...
async def test_circuit_breaker_middleware():
    ...
    circuit_options = CircuitBreakerState(failure_threshold=3, recovery_timeout_seconds=0.1)
    circuit_breaker = CircuitBreakerMiddleware(options=circuit_options, logger_factory=logger_factory)
...
async def test_metrics_middleware():
    ...
    metrics = MetricsMiddleware(report_interval_seconds=0.1, logger_factory=logger_factory)
    ...
    for _ in range(2):
        await metrics.process(
            failed_context,
            lambda ctx: failure_handler.handle(ctx.event)
        )
...
async def test_middleware_chain():
    ...
    # Create middleware components
    metrics = MetricsMiddleware(report_interval_seconds=1.0, logger_factory=logger_factory)
        ...
        return await metrics.process(
            ...
            lambda c1: circuit_breaker.process(
                ...
                lambda c2: retry.process(
                    c2,
                    lambda c3: handler.handle(c3.event)
                )
            )
        )
