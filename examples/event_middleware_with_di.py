...
from uno.core.di.container import ServiceCollection, ServiceProvider
from uno.core.di.middleware import configure_event_middleware
...
class OrderCreatedHandler(EventHandler):
    ...
    async def handle(self, context: EventHandlerContext) -> Result[str, Exception]:
        ...
        if not isinstance(event, OrderCreatedEvent):
            return Failure(ValueError(f"Expected OrderCreatedEvent, got {type(event).__name__}"))
        ...
        if self.failures <= max_failures:
            self.logger.info(f"Simulating failure in OrderCreatedHandler (attempt {self.failures})")
            return Failure(RuntimeError("Simulated temporary failure"))
        
        self.logger.info(f"Processing order {event.order_id} for customer {event.customer_id}")
...
class PaymentProcessedHandler(EventHandler):
    ...
    async def handle(self, context: EventHandlerContext) -> Result[str, Exception]:
        ...
        if not isinstance(event, PaymentProcessedEvent):
            return Failure(ValueError(f"Expected PaymentProcessedEvent, got {type(event).__name__}"))
...
class Application:
    ...
    async def create(cls) -> "Application":
        ...
        services.add_singleton(LoggerServiceFactory, implementation=LoggerServiceFactory)
        ...
        configure_event_middleware(services, registry, lambda builder: (
            ...
            .add_circuit_breaker(custom_circuit_breaker_options, [PaymentProcessedEvent.event_type])
    ...
    def _create_test_events(self):
        ...
        return order_event, order_context, payment_event, payment_context, failed_payment_event, failed_payment_context
    
    async def _process_event(self, context: EventHandlerContext, handlers: list[EventHandler], middleware: list):
        ...
        for middleware in order_middleware + payment_middleware:
            if isinstance(middleware, MetricsMiddleware):
                for event_type, metrics in middleware.metrics.items():
                    ...
                    self.logger.info(f"  Avg duration: {metrics.average_duration_ms:.2f}ms")
    ...
    async def run_demo(self) -> None:
        ...
        # Create test events
        order_event, order_context, payment_event, payment_context, failed_payment_event, failed_payment_context = self._create_test_events()
        ...
        payment_middleware = self.registry.build_middleware_chain(payment_event.event_type)
        
        # Process order event (should retry and succeed)
        self.logger.info("=== Processing Order Event (with retry) ===")
        order_results = await self._process_event(order_context, order_handlers, order_middleware)
        for result in order_results:
            self.logger.info(f"Order result: {'Success' if result.is_success else 'Failure'}")
            if not result.is_success:
                self.logger.error(f"Error: {result.error}")
        
        # Process successful payment event
        self.logger.info("\n=== Processing Successful Payment Event ===")
        payment_results = await self._process_event(payment_context, payment_handlers, payment_middleware)
        for result in payment_results:
            self.logger.info(f"Payment result: {'Success' if result.is_success else 'Failure'}")
            if not result.is_success:
                self.logger.error(f"Error: {result.error}")
        
        # Process failed payment event
        self.logger.info("\n=== Processing Failed Payment Event ===")
        failed_results = await self._process_event(failed_payment_context, payment_handlers, payment_middleware)
        for result in failed_results:
            self.logger.info(f"Failed payment result: {'Success' if result.is_success else 'Failure'}")
            if not result.is_success:
        ...
        for i in range(circuit_breaker_threshold):
            ...
            results = await self._process_event(failed_payment_context, payment_handlers, payment_middleware)
